    ...
    return (
      <div className="min-h-screen bg-background pt-24 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-text mb-2">Error Loading Campaign</h2>
          <p className="text-text-secondary mb-4">{error || 'Campaign not found'}</p>
          <button
            onClick={() => navigate(-1)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr,1fr] gap-8">
          ...
          <div className="space-y-8">
            ...
            <div className="relative rounded-xl overflow-hidden shadow-lg bg-surface">
              <img
                src={campaign.images[currentImageIndex] || 'https://images.unsplash.com/photo-1526304640581-d334cdbbf45e?auto=format&fit=crop&q=80'}
                alt={campaign.title}
            ...
            <div className="bg-surface rounded-xl p-6 shadow-lg">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-text-secondary">{campaign.category}</span>
                </div>
                <div className="flex items-center space-x-4">
                  <button
                    onClick={toggleLike}
                  ...
                  <div className="relative">
                    ...
                    >
                      {showCopiedTooltip ? <Copy className="w-5 h-5" /> : <Share2 className="w-5 h-5" />}
                    </button>
                    {showCopiedTooltip && (
              ...
              <h1 className="text-3xl font-bold text-text mb-4">{campaign.title}</h1>
              ...
              <div className="mb-6">
                <div className="h-2 bg-background-alt rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${calculateProgress(campaign.current_amount, campaign.goal)}%` }}
                ...
                <div className="flex justify-between items-center mt-2">
                  <div>
                    <div className="text-2xl font-bold text-text">
                      {campaign.current_amount.toFixed(2)} ETH
                    </div>
                    {ethPrice && (
                      <div className="text-sm text-text-secondary">
                        ≈ ${(campaign.current_amount * ethPrice).toLocaleString()} USD
                      </div>
                    )}
                  </div>
                  <div className="text-right">
                    <div className="text-lg font-medium text-text">
                      {((campaign.current_amount / campaign.goal) * 100).toFixed(1)}%
                    </div>
                    <div className="text-sm text-text-secondary">
                      of {campaign.goal.toFixed(2)} ETH goal
                    </div>
                  </div>
                </div>
              </div>
          ...
          <aside className="lg:relative">
            <div className="lg:sticky lg:top-24 space-y-6 max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-hide">
              {/* Contribution Box */}
              <div className="bg-surface rounded-xl p-6 shadow-lg">
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-text-secondary">
                    <div className="flex items-center">
                      ...
                      <span>{campaignEndDate.daysLeft} days left</span>
                    </div>
                  ...
                  <div className="flex items-center text-text">
                    ...
                    <span className="font-medium">Goal: {campaign.goal.toFixed(2)} ETH</span>
                  </div>
              ...
              {/* Creator Info */}
              <div className="bg-surface rounded-xl p-6 shadow-lg">
                <h3 className="text-xl font-bold text-text mb-4">About the Creator</h3>
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <div className="font-medium text-text">
                      {campaign.profiles?.username || 'Campaign Creator'}
                    </div>
                    <div className="text-sm text-text-secondary">
