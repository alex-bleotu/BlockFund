...
import { campaignContract } from '../../../lib/blockchain'; // Import your blockchain setup
...
export async function launchCampaign(
  ...
  try {
    ...
    const deadline = Math.floor(new Date(campaign.deadline).getTime() / 1000); // Convert deadline to Unix timestamp
    ...
    const tx = await campaignContract.createCampaign(goal, deadline, metadataCID);
